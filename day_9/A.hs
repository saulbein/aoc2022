...
main = do
  ...
  let origin = Position {x = 0, y = 0}
      initialState =
        State {headPosition = origin, tailPosition = origin, tailVisited = []}
  print
...
singleMove move state =
  State
    ...
    , tailPosition = newTailPos
    , tailVisited = newTailPos : tailVisited state
    }
  where
    direction =
      case move of
        ...
        L -> ToLeft
        R -> ToRight
    newHeadPos = headPosition state & moveTowards direction
    newTailPos = followTo (tailPosition state) newHeadPos
