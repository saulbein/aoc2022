...
main = do
  ...
  let parsed = contents & parseContent
      result =
        parsed & zip [1 ..] & filter (\(i, pair) -> isCorrectOrder pair) &
        map fst &
        sum
  print result
...
parseContent :: String -> [(Packet, Packet)]
parseContent content = content & lines & filter (/= "") & map parseLine & pairs
  where
    parseLine :: String -> Packet
    parseLine line = tokenize line & parseList & fromJust & snd
    pairs :: [a] -> [(a, a)]
    pairs [] = []
    pairs [x] = error "Cannot pair single value"
...
isCorrectOrder (left, right) = left <= right
